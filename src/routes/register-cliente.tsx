  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: { full_name: nombre, portal: "cliente" },
      },
    });
    if (error) { setLoading(false); toast.error(error.message); return; }
    if (data.session) {
      try { await assignRole({ data: { role: "cliente" } }); } catch (e: any) { console.error(e); }
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary-soft to-background px-4 py-10">
      <div className="w-full max-w-md rounded-xl border bg-card p-8 shadow-lg">
        ...
        <h1 className="text-center text-2xl font-bold text-primary-dark">Crear cuenta Cliente</h1>
        <p className="mt-1 text-center text-sm text-muted-foreground">Solicita transporte con TN Chile.</p>
