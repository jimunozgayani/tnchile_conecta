DROP FUNCTION IF EXISTS public.upsert_disponibilidad_dia(uuid, date, text, uuid, text, uuid, text, text, uuid, text, text);
...
BEGIN
  ...
  INSERT INTO public.disponibilidad_chofer (
    ...
    modalidad, tipo_camion_id, tipo_camion_otro, truck_id, notas, fuente, created_by
  ) VALUES (
    _driver_id, _fecha, _fecha, COALESCE(_estado, 'disponible'),
    _lugar_ciudad_id, _lugar_texto, _destino_ciudad_id, _destino_texto,
    _modalidad, _tipo_camion_id, _tipo_camion_otro, v_truck_id, _notas,
  ...
  DO UPDATE SET
    ...
    notas = CASE WHEN _notas IS NULL THEN disponibilidad_chofer.notas
                 WHEN _notas = '' THEN NULL ELSE _notas END,
...
REVOKE ALL ON FUNCTION public.upsert_disponibilidad_dia(uuid, date, text, uuid, text, uuid, text, text, uuid, text, text, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.upsert_disponibilidad_dia(uuid, date, text, uuid, text, uuid, text, text, uuid, text, text, text) TO authenticated;